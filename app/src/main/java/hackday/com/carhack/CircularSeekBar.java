...
public class CircularSeekBar extends View {
    ...
    protected float circle = 180f;
    protected float initPointerProgress = 90;
    ...
    protected void initAttributes(TypedArray attrArray) {
        ...
        mStartAngle = ((circle + (attrArray.getFloat((R.styleable.CircularSeekBar_start_angle), DEFAULT_START_ANGLE) % circle)) % circle);
        mEndAngle = ((circle + (attrArray.getFloat((R.styleable.CircularSeekBar_end_angle), DEFAULT_END_ANGLE) % circle)) % circle);
    ...
    protected void calculateTotalDegrees() {
        mTotalCircleDegrees = (circle - (mStartAngle - mEndAngle)) % circle; // Length of the entire circle/arc
        if (mTotalCircleDegrees <= 0f) {
            mTotalCircleDegrees = circle;
        }
    }
    ...
    protected void calculateProgressDegrees() {
        ...
        mProgressDegrees = (mProgressDegrees < 0 ? circle + mProgressDegrees : mProgressDegrees); // Verified
    }
    ...
    protected void calculatePointerAngle() {
        ...
        mPointerPosition = mPointerPosition % circle;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        cwDistanceFromStart = (cwDistanceFromStart < 0 ? circle + cwDistanceFromStart : cwDistanceFromStart); // Verified
        ccwDistanceFromStart = circle - cwDistanceFromStart; // Verified

        cwDistanceFromEnd = touchAngle - mEndAngle; // Verified
        cwDistanceFromEnd = (cwDistanceFromEnd < 0 ? circle + cwDistanceFromEnd : cwDistanceFromEnd); // Verified
        ccwDistanceFromEnd = circle - cwDistanceFromEnd; // Verified
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                cwDistanceFromPointer = (cwDistanceFromPointer < 0 ? circle + cwDistanceFromPointer : cwDistanceFromPointer);
                ccwDistanceFromPointer = circle - cwDistanceFromPointer;
